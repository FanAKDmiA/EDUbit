            ...
            <div className="flex flex-wrap justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm">
                  Rol solicitado: <span className="font-semibold">{roleLabel[request.requested_role as keyof typeof roleLabel]}</span>
                </p>
                <p className="text-sm text-ink/65">Estado: {request.status}</p>
                {request.institution ? <p className="text-sm text-ink/65">Institución: {request.institution}</p> : null}
                {request.course_reference ? <p className="text-sm text-ink/65">Curso: {request.course_reference}</p> : null}
